...
CASES = max(1, int(os.getenv('RANDOMIZER_CASES', '30') or 30))
PRESETS = ['high', 'extremely_high', 'extremely_high', 'high', 'medium_high']
...
def main():
    ...
    for idx in range(CASES):
        ...
        horizon = 3 if idx < 10 else (4 if idx < 20 else 5)
        stress_mode = True
        ...
        scenario = req('POST', f'{API}/admin/scenarios', headers=headers, payload={'name': f'AUTO_30_ESC_{idx+1}_{int(time.time())}'}, timeout=30)
        scenario_id = int(scenario['id'])

        payload = {
            'preset': preset,
            'seed': 20260304 + 500 + idx,
        ...
        try:
            preview = req('POST', f'{API}/admin/scenarios/{scenario_id}/randomizer/preview', headers=headers, payload=payload, timeout=180)
            _ = req('POST', f'{API}/admin/scenarios/{scenario_id}/randomizer/apply', headers=headers, payload=payload, timeout=180)
            run_resp = requests.post(f'{API}/admin/scenarios/{scenario_id}/run', headers=headers, json={'scope_mode': 'focused'}, timeout=420)
            run_resp.raise_for_status()

            runs = req('GET', f'{API}/admin/scenarios/{scenario_id}/runs', headers=headers, timeout=120)
            run_id = int(runs[0]['id']) if runs else None
            summary = req('GET', f'{API}/admin/scenarios/{scenario_id}/runs/{run_id}/summary', headers=headers, timeout=180) if run_id else {}
            ...
            neighbor_alloc = float(allocations.get('neighbor_state') or allocations.get('neighbor') or 0.0)
    ...
    out_json = Path(f'RANDOMIZER_STRESS_ESCALATION_30_CASE_REPORT_{stamp}.json')
    out_md = Path(f'RANDOMIZER_STRESS_ESCALATION_30_CASE_REPORT_{stamp}.md')
